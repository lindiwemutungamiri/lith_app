...
import 'package:lith_app/screens/patient_dashboard/my_diary/my_diary_screen.dart';
        ...
        builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
          if (snapshot.hasData) {
            ...
            for (var i = 0; i < snapshot.data!.docs.length; i++) {
              ...
              alldatacarbo = alldatacarbo +
                  double.parse(snapshot.data!.docs[i]["Total Carbohydrate"]);
              alldataprotein = alldataprotein +
                  double.parse(snapshot.data!.docs[i]["Total Protein"]);
              alldatafat = alldatafat +
                  double.parse(snapshot.data!.docs[i]["Total Fat"]);
            }
          }
          return StreamBuilder<QuerySnapshot>(
              stream: FirebaseFirestore.instance
                ...
                if (snapshot.hasData) {
                  ...
                  for (var i = 0; i < snapshot.data!.docs.length; i++) {
                    alltargetcarbs = alltargetcarbs +
                        double.parse(
                            snapshot.data!.docs[i]["Target Carbohydrate"]);
                    alltargetprotein = alltargetprotein +
